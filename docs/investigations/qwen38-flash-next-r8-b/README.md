...
3. **PLE host residency is the runtime default and was demonstrated.** At this revision `TENSOR_READ_LAZY` + auto-lazy (>4 GiB, mmap) puts `per_layer_token_embd.weight` (27465 MiB) in a client-side CPU mmap; `-ot` cannot move it (lazy return precedes the override block — matches R8-A's source audit). Both arms' load logs retain the lazy-read line; accounting shows the client's PLE is file-backed (`RssFile`), with NO anonymous host mirror.
