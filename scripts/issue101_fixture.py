...
def execute(plan, participant, node, coordinator):
    ...
    if participant["node_id"] != node.node_id:
        from issue99_artifact_core import fail
        raise fail("RECONCILIATION_MISMATCH", "execution Node identity")
    identity = {"epoch": plan["epoch"], "participant_id": participant["participant_id"],
    ...
    for record in participant["required_artifacts"]:
        ...
        materializations.append({**identity, "logical_state_id": state,
    ...
    # The fixture state names specify independent coefficient values.
    reference = sum(int(state.removeprefix("coefficient.")) * 2 ** index
    ...
    reconciliation = coordinator.reconcile(
        ...
        materializations)
    return {**identity, "output": output, "reference": reference,
