#!/usr/bin/env python3
"""Generate the deterministic v4 corpora frozen by InferSwarm issue #86.
...
the seeds, case-id prefixes (c95-/p95-/h95-), schema versions, and the v4
...
from issue95_v4_methodology import (
    ...
    V4_STRESS_POOL_CASES,
    V4_STRESS_POOL_SCHEMA,
    V4_STRESS_POOL_SEED,
)

V4_DISJOINTNESS_NOTE = (
    "v4 artifacts are mechanically disjoint by prompt_sha256 and "
    "token_ids_sha256 from: the c74-* calibration corpus, the v1 stress "
    "pool, the v2 stress pool, the historical #74 holdout public "
    "commitment, and the fresh v4 stress pool / v4 holdout commitment"
)
...
def generate_calibration(tokenizer: Any) -> dict[str, Any]:
    ...
    for class_name in CONTENT_CLASSES:
        for regime_index in range(len(LENGTH_REGIMES)):
            for index in range(V4_CASES_PER_CELL):
                cases.append(_case(V4_CALIBRATION_SEED, "calibration", "c95-", tokenizer,
                                   class_name, regime_index, index))
    return {
        "schema": V4_CALIBRATION_SCHEMA,
        "contract_id": CONTRACT_ID,
        "generator": "scripts/generate_issue95_corpora.py",
        ...
        "seed": V4_CALIBRATION_SEED,
        "cases_per_cell": V4_CASES_PER_CELL,
        "cases": cases,
        "disjointness": V4_DISJOINTNESS_NOTE,
    }
    ...
    for class_name in CONTENT_CLASSES:
        for regime_index in range(len(LENGTH_REGIMES)):
            for index in range(2):
                cases.append(_case(V4_STRESS_POOL_SEED, "stress", "p95-", tokenizer,
                                   class_name, regime_index, index))
    assert len(cases) == V4_STRESS_POOL_CASES
    return {
        "schema": V4_STRESS_POOL_SCHEMA,
        "contract_id": CONTRACT_ID,
        "generator": "scripts/generate_issue95_corpora.py",
        "generator_sha256": sha256_file(Path(__file__)),
        "tokenizer": {
...
def generate_holdout(tokenizer: Any, secret_seed: str) -> dict[str, Any]:
    ...
    for class_name in CONTENT_CLASSES:
        for regime_index in range(len(LENGTH_REGIMES)):
            cases.append(_case(secret_seed, "v4-sealed-holdout", "h95-", tokenizer,
                               class_name, regime_index, 0))
    assert len(cases) == V4_HOLDOUT_CASES
    from issue95_v4_methodology import sha256_bytes
    return {
        "schema": V4_HOLDOUT_PLAINTEXT_SCHEMA,
        "contract_id": CONTRACT_ID,
        "generator": "scripts/generate_issue95_corpora.py",
        "generator_sha256": sha256_file(Path(__file__)),
