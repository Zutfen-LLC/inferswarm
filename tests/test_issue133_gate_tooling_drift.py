...
controls (maintainer review 5166773760, extended by review 5167622668).

These tests cover the ACCEPTED-MATERIALIZATION gate-tooling closure
seam (defense in depth beneath the external bootstrap of review
5167622668; the adversarial working-tree-substitution controls live
in tests/test_issue133_prelaunch_bootstrap.py):
...
class _ScratchFixture:
    """An accepted-history scratch repository: base commit carrying
    the complete corrected gate-tooling closure and retained evidence
    (EXCLUDING the authority document), then the authority-bearing
    commit whose tree ALSO carries the regenerated /6 execution freeze
    bound into the authority document, with refs/remotes/origin/main
    pointing at it — the minimum accepted history in which the full
    pre-execution gate can pass.

    Mirrors production ordering: the real unmocked builder dry run is
    executed from the fixture bytes and the freeze regenerated by
    scripts/issue133_regenerate_corrected_freeze.py BEFORE the
    authority-bearing commit is finalized.
    """
    ...
    def __init__(self, tmp: str):
        ...
        (self.repo / "_issue133_dry_run_probe.py").write_text(
            _DRY_RUN_PROBE)
        self.env = dict(
            os.environ,
        ...
        # authority commit is unambiguous; the regenerated /6 freeze
        # is folded into the SAME authority-bearing commit (production
        # ordering: code committed, builder dry run executed, freeze
        # regenerated and rebound, history finalized)
