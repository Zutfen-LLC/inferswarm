#!/usr/bin/env python3
"""Issue #228 — V2-E host-observation helpers (inferswarm02).

Adapted from the accepted #216 host module (same fail-closed receipt
discipline: read-only probes retain structured receipts; a missing
receipt or nonzero exit is an ERROR at collection time; unavailable
telemetry fields are None, never estimated). Adds the V2-E route
instrumentation: per-BDF sysfs link state + AER counters for the whole
root->switch->die path, and host-link counter samples.
"""
