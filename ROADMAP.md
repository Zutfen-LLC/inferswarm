...
- **Recorded execution authorization:** blocked — Issue #133 — physical Arm-C retry campaign terminal ISSUE117_ARM_C_ORDINARY_SERVING_FAIL ACCEPTED via PR #136 merge 1b83bcab0a5e682a438ca0554f71dd0ace15be55 (attempt armc-retry-physical-1, campaign armc-retry-afcdc4428f95d50c; 18/24 exact equality, six regime-4 committed-token divergences re-observed on freshly executed arms with mechanically derived first-divergence positions (c109-04-01-026:4, c109-04-02-047:0, c109-04-03-040:2, c109-04-04-024:3, c109-04-05-043:0, c109-04-06-074:0) and complete pre-divergence invocation-equivalence proof; the four regime-4 HTTP-content discrepancies are retained as non-prefix-stable incremental decoding; all Coordinator-zero, fencing, participant data-path, and deployment-identity invariants mechanically derived). Observation accepted as final historical evidence; Arm D blocked by the FAIL terminal pending a separate remediation plus requalification authority.. [Authority](https://github.com/Zutfen-LLC/inferswarm/issues/133).
...
- Issue #137 Arm-C regime-4 diagnosis DIAGNOSTIC_ONLY, corrected 2026-09-11 (PR #138 correction pass): OBSERVED terminal ISSUE117_ARM_C_REGIME4_DIAGNOSIS_PARTIAL derived by the corrected fail-closed reducer from corrected evidence — the corrected one-variable intervention (probe C2: fresh paired substrates per trial, counterbalanced arm order) proves multi-chunk extend-prefill execution sufficient for committed-token instability on an otherwise stable input while the single-chunk path is bit-deterministic; earliest observed divergence stable embedding_output to varying after_layer_0 on the captured case c109-04-02-047; prior request history not necessary for divergence; a single identical numerical mechanism for all six cases is NOT established — three cases vary in-session, three are session-stable matching no accepted value, so the population shares a necessary path without proven numerical-mechanism identity. ACCEPTANCE: not yet recorded — the PR remains open for maintainer review; no maintainer acceptance is pre-recorded here. EXECUTION AUTHORIZATION: diagnosis only; no remediation authorized or landed; the accepted #133 FAIL is unchanged; Arm D stays blocked pending a separate remediation plus requalification authority.
