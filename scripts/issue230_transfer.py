#!/usr/bin/env python3
"""Issue #230 — V2-F external-memory transfer producer (embedded C).

THE reviewed transfer implementation the #228 terminal said did not
exist. Physical transfer execution is ENABLED for this campaign, but
only through this module's frozen machinery, under the frozen ladder,
and behind the frozen safety gate (issue230_safety) and order-of-execution
state machine. The producer deletes none of the #228 fences: the #228
campaign's own refusal path stays as-is on its own branch history.

Execution-device authority (mandatory after the rejected #228 attempt-1
producer):

* the embedded C creates ONE Vulkan instance with an explicit
  VkApplicationInfo (API 1.1);
* it enumerates physical devices and derives each device's PCI BDF from
  its VkPhysicalDeviceIDProperties deviceUUID (the RADV UUID encodes
  the BDF), then selects source and destination BY EXPECTED BDF taken
  from the retained fresh mapping — never by enumeration order or name
  substring;
* it creates TWO SEPARATE logical devices, one per physical device, and
  submits every command through the OWNING logical device's queue. A
  queue label or enumeration index is never execution authority; the
  retained per-rep record states which physical device (UUID+BDF)
  executed source-side and destination-side work;
* the actual inter-die transfer is a destination-side
  vkCmdCopyBuffer(dst_local <- src_imported) submitted on the
  destination device after the source-side fill fence has completed
  (sequential-fence synchronization; no concurrent access; no implicit
  CPU staging between the dies — the source bytes move only through the
  imported-memory view).

Handle-type independence: opaque_fd and dma_buf are separate arms with
separately enabled extensions and distinct handle-type bits in the
export/import chains. Success of one is never evidence for the other.

Fail-closed everywhere: every VkResult is checked and retained with its
exact stage; any Vulkan failure exits nonzero with a structured stderr
line and produces NO functional row; fd ownership is tracked explicitly
(opaque_fd: consumed by the driver on successful import; dma_buf:
caller closes after import; failure paths always close).

Subcommands of the compiled probe:

  identity <bdf_a> <bdf_b>
  transfer <mechanism> <direction> <bdf_src> <bdf_dst> <size> <reps> <warmups> <seed>
  samedie  <bdf> <size> <reps> <warmups> <seed>          (control)
  hoststaged <bdf_src> <bdf_dst> <size> <reps> <warmups> <seed>  (control)
  linkio   <bdf> <size> <reps> <warmups> <seed>           (control)

Output: one JSON object per line on stdout (identity / rep / summary
events); errors exit nonzero with a JSON error line on stderr.
"""
...
SCHEMA_TRANSFER_ARM = "inferswarm.v2f.transfer-arm/1"
...
#: Stages at which a Vulkan failure is retained (fail-closed evidence).
STAGES = (
    "instance", "enumerate", "select_source", "select_dest",
    "device_source", "device_dest", "buffer_source", "memory_source_export",
    "fd_export", "fd_import_properties", "memory_dest_import",
    "buffer_dest_view", "buffer_dest_local", "staging_source",
    "staging_dest", "cmdpool_source", "cmdpool_dest", "fill_source",
    "copy_dest", "verify_readback", "cleanup",
)


class ProbeError(RuntimeError):
    """The transfer probe could not be compiled/executed/parsed."""


# ---------------------------------------------------------------------------
# Embedded C transfer probe.
# ---------------------------------------------------------------------------
...
# C syntax check via gcc -fsyntax-only when headers exist locally.


def _gcc_available() -> bool:
    try:
        subprocess.run(["gcc", "--version"], capture_output=True, timeout=30)
        return True
    except (OSError, subprocess.TimeoutExpired):
        return False


def compile_transfer(build_dir: Path, source_override: str | None = None,
                     extra_cflags: list[str] | None = None
                     ) -> tuple[Path, str]:
    """Compile the transfer probe; return (binary, source_text).

    ``source_override`` lets contract tests compile a MUTATED source
    (e.g. the wrong-handle-type defect) against a stub header.
    """
    build_dir.mkdir(parents=True, exist_ok=True)
    src = source_override if source_override is not None else _C_TRANSFER
    src_path = build_dir / "issue230_transfer.c"
    src_path.write_text(src, encoding="utf-8")
    bin_path = build_dir / "issue230_transfer"
    # -lvulkan needs libvulkan-dev (present on inferswarm02); hosts with
    # only libvulkan1 still link against the versioned soname.
    base_cmd = ["gcc", "-O1", "-g", "-std=c11", "-Wall", "-Wextra",
                "-o", str(bin_path), str(src_path)]
    link_variants: list[list[str]] = [
        base_cmd + ["-lvulkan"],
        base_cmd + ["/usr/lib/x86_64-linux-gnu/libvulkan.so.1"],
    ]
    if extra_cflags:
        link_variants = [["gcc"] + extra_cflags + c[1:] for c in link_variants]
    proc = subprocess.run(link_variants[0], capture_output=True, timeout=180)
    for cmd in link_variants:
        proc = subprocess.run(cmd, capture_output=True, timeout=180)
        if proc.returncode == 0:
            return bin_path, src
    assert proc is not None
    raise ProbeError(
        f"transfer probe compile failed:\n{proc.stderr.decode()[:2000]}")


def run_probe(binary: Path, argv: list[str], raw_dir: Path, name: str,
              timeout: int = 900, env: dict[str, str] | None = None,
              ) -> dict[str, Any]:
    """Run one probe invocation; retain raw bytes durably; never hide a
    nonzero exit (the caller decides stop semantics)."""
    run_env = None
    if env:
        run_env = dict(env)
    t0 = time.monotonic()
    ...
    host.durable_write(raw_dir / f"{name}.stdout", stdout.encode())
    host.durable_write(raw_dir / f"{name}.stderr", stderr.encode())
    host.durable_write(raw_dir / f"{name}.exit-code",
                       f"{proc.returncode}\n".encode())
    return {
        "name": name,
        "argv": argv,
        ...
        "stderr_rel": f"raw/{name}.stderr",
        "stdout_sha256": hashlib.sha256(stdout.encode()).hexdigest(),
        "stderr_sha256": hashlib.sha256(stderr.encode()).hexdigest(),
    }


def parse_transfer_stdout(stdout: str) -> dict[str, Any]:
    """Parse the probe's JSON-lines output into a structured record.

    Fail closed: any line that is not valid JSON, a missing arm record,
    or an inconsistent rep population raises ProbeError.
    """
    arm = None
    ...
    for line in stdout.splitlines():
        ...
        if ev == "arm":
            ...
            error = obj
    if arm is None:
        raise ProbeError("no arm record in probe output")
    return {"arm": arm, "reps": reps, "summary": summary, "error": error}


def validate_transfer_run(parsed: dict[str, Any], *, mechanism: str,
                          direction: str, size: int, reps_expected: int,
                          warmups_expected: int) -> dict[str, Any]:
    """Validate one transfer arm's parsed output against the frozen
    expectations. Returns the validated record or raises ProbeError.

    Controls (issue list): nonzero exit hidden by summary; dropped or
    duplicate repetitions; missing correctness observations; A->B
    evidence reused for B->A (direction must match every record).
    """
    arm = parsed["arm"]
    if arm.get("mechanism") != mechanism:
        raise ProbeError(f"arm mechanism {arm.get('mechanism')!r} != "
                         f"requested {mechanism!r}")
    if arm.get("direction") != direction:
        raise ProbeError(
            f"arm direction {arm.get('direction')!r} != requested "
            f"{direction!r} (cross-direction evidence reuse is fatal)")
    if int(arm.get("size", -1)) != size:
        raise ProbeError(f"arm size {arm.get('size')!r} != {size}")
    measured = [r for r in parsed["reps"] if r.get("measured")]
    warm = [r for r in parsed["reps"] if not r.get("measured")]
    if len(measured) != reps_expected:
        raise ProbeError(f"measured rep count {len(measured)} != "
                         f"{reps_expected} (dropped/duplicated reps)")
    if len(warm) != warmups_expected:
        raise ProbeError(f"warmup count {len(warm)} != {warmups_expected}")
    rep_ids = sorted(int(r["rep"]) for r in measured)
    if rep_ids != list(range(reps_expected)):
        raise ProbeError(f"measured rep ids {rep_ids} not exactly "
                         f"0..{reps_expected - 1}")
    for r in parsed["reps"]:
        if r.get("ok") is not True:
            raise ProbeError(
                f"rep {r.get('rep')} carries no exact-true correctness "
                "observation (missing/ok=false rows are never functional)")
        if "seed" not in r:
            raise ProbeError(f"rep {r.get('rep')} lacks its seed identity")
    for r in measured:
        ns = r.get("elapsed_ns")
        if not isinstance(ns, (int, float)) or ns <= 0:
            raise ProbeError(
                f"measured rep {r.get('rep')} lacks positive timing")
    if parsed["error"] is not None:
        raise ProbeError(f"probe emitted an error record: "
                         f"{parsed['error']}")
    return {
        "arm": arm,
