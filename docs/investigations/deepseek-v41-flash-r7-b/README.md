...
Status: CPU/static source audit for [issue #209](https://github.com/Zutfen-LLC/inferswarm/issues/209). This additive correction preserves the accepted R7-A bundle byte-for-byte and retains both the insufficient one-file vLLM disposition at `69e07e5` and the expanded-source terminal correction at `9feb7e74`; their history and reasons remain in `superseded-69e07e5.json` and `superseded-9feb7e74.json`.

## Runtime decision

`runtime-authority.json` records vLLM `0eae9acd4d01574e12d4ecf6a0229813f7fdb799` as `EVIDENCE_BLOCKED` only on p8 (`observable_cache_authority`), which is `UNPROVEN`. p1–p7, p9, and p10 are `PASS`. `external-source-evidence.json` pins its Apache-2.0 license, build configuration, full-file SHA-256 identities, paths, and decisive excerpts for the DeepSeek V4.1 model, PP utilities, standard safetensors loader, and attention/cache implementation.
