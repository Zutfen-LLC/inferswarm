...
- **Recorded execution authorization:** blocked — Arm-C chunk-policy remediation is implemented and CPU-proven as ISSUE117_ARM_C_REMEDIATION_READY under issue #153 (classification UNNECESSARY_PARTITION_POLICY; FreeToken remediation producer open for maintainer review). No requalification execution is authorized: accept this remediation, then create a fresh Arm-C requalification authority that freezes the candidate producer before any physical run. Arm D remains blocked until an accepted ISSUE117_ARM_C_ORDINARY_SERVING_PASS.
...
- Issue #153 remediation is implemented as ISSUE117_ARM_C_REMEDIATION_READY (UNNECESSARY_PARTITION_POLICY): a legal at-or-below-64-row extend-prefill unit provably remains one backend call on the canonical serving seam; the remediation producer is a candidate, not accepted execution authority.
