#!/usr/bin/env python3
"""Select the eight v4 stress cases from reference-only margins (issue #86).
...
- finite NEGATIVE margin: unconditional reference/order-consistency failure,
  fails closed immediately (NEGATIVE_REFERENCE_MARGIN) — new in v4;
...
def select(pool: dict[str, Any], margins: dict[str, Any], commitment: dict[str, Any]) -> dict[str, Any]:
    ...
    if margins.get("schema") != V4_MARGIN_SUMMARY_SCHEMA:
        raise ValueError("v4 reference margin summary schema mismatch")
    ...
    if commitment.get("schema") != V4_STRESS_COMMITMENT_SCHEMA:
        raise ValueError("v4 stress selection commitment schema mismatch")
    if commitment.get("contract_id") != CONTRACT_ID:
        raise ValueError("v4 stress selection commitment contract mismatch")
    if commitment.get("state") != V4_COMMITMENT_STATE:
        raise ValueError("v4 stress selection rule was not committed before reference execution")
    if commitment.get("candidate_pool_sha256") != pool_hash:
        raise ValueError("v4 stress selection commitment does not bind the exact frozen pool")
    if commitment.get("candidate_observations_forbidden") is not True:
        raise ValueError("v4 stress selection commitment does not forbid candidate observations")
    if commitment.get("selected_case_count") != 8:
        raise ValueError("v4 stress selection commitment must require eight cases")
    if commitment.get("selection_program") != "scripts/select_issue95_margin_stress_v4.py":
        raise ValueError("v4 stress selection commitment program path mismatch")
    if Path(commitment["selection_program"]).name and commitment.get(
        "selection_program_sha256"
    ) != sha256_file(Path(__file__).resolve().parent / "select_issue95_margin_stress_v4.py"):
        raise ValueError("v4 stress selection commitment program hash mismatch")
    if commitment.get("eligibility_rule") != V4_ELIGIBILITY:
        raise ValueError("v4 stress selection commitment eligibility rule mismatch")
    if commitment.get("minimum_eligible_cases") != MIN_ELIGIBLE:
        raise ValueError("v4 stress selection commitment minimum eligible count mismatch")
    if commitment.get("margin_definition") != MARGIN_DEFINITION:
        raise ValueError("v4 commitment margin definition is not the unchanged min-over-8 definition")
    if commitment.get("selection_rule") != V4_SELECTION_RULE:
        raise ValueError("v4 commitment selection rule mismatch")
    if margins.get("stress_pool_sha256") != pool_hash:
        raise ValueError("v4 reference margins do not bind the exact frozen pool")
    pool_cases = {row["case_id"]: row for row in pool["cases"]}
    rows = margins.get("cases")
    if not isinstance(rows, list) or not rows:
        raise ValueError("v4 reference margins must be a nonempty list")
    if {row.get("case_id") for row in rows} != set(pool_cases) or len(rows) != len(pool_cases):
        raise ValueError("v4 reference margins must cover each pool case exactly once")
    ...
    for row in rows:
        ...
        if case_id in seen_ids:
            raise ValueError(f"v4 reference margins contain duplicate case ID {case_id!r}")
    ...
    if len(eligible) < MIN_ELIGIBLE:
        raise ValueError(
            f"v4 stress selection requires at least {MIN_ELIGIBLE} eligible cases; "
    ...
    return {
        "schema": V4_SELECTED_EIGHT_SCHEMA,
