#!/usr/bin/env python3
"""InferSwarm issue #86: freeze the v3 stress-selection commitment.

Creates the PUBLIC commitment binding the frozen v3 selector program,
eligibility rule, and margin definition to the exact frozen p86-* pool,
BEFORE any reference observation. Mirrors the accepted v2 commitment shape
(issue #76 v2 / PR #78) with v3 eligibility per issue #86 section 2:
...
- finite ZERO margin: ELIGIBLE exact-tie stress case (v3 change);
...
SELECTOR_PATH = "scripts/select_issue86_margin_stress_v3.py"
...
def build_commitment(pool: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": V3_STRESS_COMMITMENT_SCHEMA,
        "contract_id": CONTRACT_ID,
        "state": V3_COMMITMENT_STATE,
