#!/usr/bin/env python3
"""Create the public commitment + custody record for the sealed v3 holdout.
...
from issue74_methodology import CONTRACT_ID, canonical_json_bytes, sha256_file
from issue86_v3_methodology import (
    V3_HOLDOUT_COMMITMENT_SCHEMA,
    V3_HOLDOUT_CUSTODY_SCHEMA,
...
def build_commitment(
    ...
    if plaintext_holdout.get("schema") != V3_HOLDOUT_PLAINTEXT_SCHEMA:
        raise ValueError("holdout plaintext schema mismatch")
    cases = plaintext_holdout.get("cases")
    if not isinstance(cases, list) or len(cases) != 24:
        raise ValueError("v3 holdout must contain exactly 24 cases")
    observed_cells = {
    ...
    if observed_cells != expected_cells:
        raise ValueError("v3 holdout must contain exactly one case in each of 24 cells")
    ...
    return {
        "schema": V3_HOLDOUT_COMMITMENT_SCHEMA,
        ...
        "unseal_rule": (
            "only after the v3 threshold manifest is committed and the v3 "
...
def build_custody_record(
    ...
    return {
        "schema": V3_HOLDOUT_CUSTODY_SCHEMA,
        "contract_id": CONTRACT_ID,
        ...
        "fail_closed_rule": (
            "if both custodians become unavailable the v3 methodology terminates "
