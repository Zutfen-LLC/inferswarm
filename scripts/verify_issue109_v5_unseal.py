#!/usr/bin/env python3
"""Non-decrypting issue #109 v5 unseal preflight.

OpenSSL is used only to derive public-key DER from supplied key/certificate;
no CMS decrypt operation is present or reachable. Fails closed unless the
custody record shows holdout_state == SEALED_NOT_CONSUMED with at least two
independently verified custodians (see commit_issue109_holdout.py).
"""
...
def validate_unseal_preconditions(*, core_threshold_path: Path, expected_core_threshold_sha256: str,
    ...
    commitment = json.loads((V5 / 'manifests/sealed-holdout-commitment.json').read_text())
    if threshold.get('schema') != 'inferswarm.issue109.v5-core-threshold-manifest/1' or threshold.get('holdout_state') != 'SEALED_NOT_CONSUMED':
