...
def execution_delta_audit(freetoken: Path) -> dict:
    ...
    return {
        ...
        "delta_statement": (
            ...
            "producer lineage beyond the accepted Arm-C producer is the "
            "accepted #153 single-chunk policy and the accepted #166 "
            "session-SWA-lifecycle ownership (hashes bound here; identical "
            "set to the accepted #168 audit — no new delta exists between "
            "the #168-audited head and this campaign's head because they "
            "are the same FreeToken commit)"),
    }
...
def main() -> int:
    ...
    record = {
        ...
        "plan_identities": {
            ...
            "r5a_static_plan_digest": P.AUTHORIZED_R5A_STATIC_PLAN_DIGEST,
