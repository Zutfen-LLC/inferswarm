#!/usr/bin/env python3
"""InferSwarm issue #86: Gemma v3 decision-stability methodology (CPU-only).

Implements the accepted issue #83 semantic contract as the first Gemma v3
...
# semantics are re-defined here as v3 identities, not reused.
...
METHODOLOGY_ID = "inferswarm.issue86.v3-methodology/1"
ISSUE83_ACCEPTED_SHA = "d60b8f6c4490c91312e8d073b4ac55794bf68841"

# --- frozen v3 corpora identities (issue #86 section 1/2/10) ---------------

V3_CALIBRATION_SEED = "inferswarm-issue-86-calibration-v3"
V3_STRESS_POOL_SEED = "inferswarm-issue-86-stress-pool-v3"
V3_CALIBRATION_SCHEMA = "inferswarm.issue86.v3-calibration-corpus/1"
V3_STRESS_POOL_SCHEMA = "inferswarm.issue86.v3-stress-pool/1"
V3_HOLDOUT_PLAINTEXT_SCHEMA = "inferswarm.issue86.v3-holdout/1"
V3_HOLDOUT_COMMITMENT_SCHEMA = "inferswarm.issue86.v3-holdout-commitment/1"
V3_HOLDOUT_CUSTODY_SCHEMA = "inferswarm.issue86.v3-holdout-custody-record/1"
V3_STRESS_COMMITMENT_SCHEMA = "inferswarm.issue86.v3-stress-selection-commitment/1"
V3_MARGIN_SUMMARY_SCHEMA = "inferswarm.issue86.v3-reference-margin-summary/1"
V3_SELECTED_EIGHT_SCHEMA = "inferswarm.issue86.v3-selected-stress-eighth/1"
V3_COMMITMENT_STATE = "COMMITTED_BEFORE_MATCHED_REFERENCE_EXECUTION"
V3_SELECTION_STATE = "FROZEN_AFTER_MATCHED_REFERENCE_BEFORE_HETEROGENEOUS_CANDIDATE"
V3_STATISTICAL_CASES = 576
V3_STRESS_POOL_CASES = 48
V3_HOLDOUT_CASES = 24
...
def decision_domain_construction_identity() -> str:
    """The frozen v3 decision-domain construction rule identity (issue #86 §5)."""
...
def derive_e_d(statistical_case_e_ds: Sequence[float], stress_case_e_ds: Sequence[float]) -> float:
    ...
    statistical arm: exactly the 576 v3 statistical cases;
    stress arm: exactly the 8 committed selected stress cases.
    """
    if len(statistical_case_e_ds) != V3_STATISTICAL_CASES:
        raise MethodologyError(
            f"the statistical E_D arm requires exactly {V3_STATISTICAL_CASES} cases"
        )
    if len(stress_case_e_ds) != V3_SELECTED_STRESS_CASES:
        raise MethodologyError(
            f"the stress E_D arm requires exactly {V3_SELECTED_STRESS_CASES} cases"
        )
...
SIMULTANEOUS_FAMILIES = 16  # 15 numerical envelope families + E_D
BONFERRONI_FAMILIES = SIMULTANEOUS_FAMILIES


def minimum_sample_size_v3(
...
def statistical_design_v3() -> dict[str, Any]:
    """The machine-reproducible 16-family statistical design record."""
    minimum = minimum_sample_size_v3()
    return {
        ...
        "families": "15 numerical envelope families + the calibration-derived E_D family",
        ...
        "minimum_n": minimum,
        "selected_n": V3_STATISTICAL_CASES,
        "design": "balanced 24 cells x 24 independent cases",
        "independent_unit": "the case, not the token row",
        "sufficiency": (
            f"576 >= {minimum}; within-case decision/checkpoint values are reduced "
...
def evaluate_decision(
    ...
    *,
    ...
    """Evaluate ONE canonical-prefix decision under the frozen v3 gate order.
    ...
    """
    _check_v3_frozen_identity(tie_break_identity, argmax_tie_break_identity(), "argmax/tie-break")
    _check_v3_frozen_identity(domain_identity, decision_domain_construction_identity(), "decision-domain construction")
    if len(reference_logits) != len(candidate_logits) or not reference_logits:
        raise MethodologyError("reference and candidate rows must share a nonzero size")
