...
- **Recorded execution authorization:** blocked — Arm-C SWA allocation remediation implemented under issue #166 (terminal ISSUE117_ARM_C_SWA_REMEDIATION_READY, implementation-remediation-only, no physical execution): the standalone R6 stage runtime now owns the session SWA allocation lifecycle — incremental alloc_swa frontier established before each chunk's first prefix-consuming SWA KV operation, mapping persistence across chunks/decode, wholesale release on reset, fail-closed exhaustion/gaps — the same underlying allocation semantic as the accepted #157 intervention and the scheduler path, proven CPU-only with lifecycle and negative-control tests. The remediation producer is NOT promoted to qualification authority; fresh Arm-C requalification remains separately blocked pending maintainer acceptance/merge of the remediation. Arm C remains failed; no requalification executed; Arm D/E remain blocked.
