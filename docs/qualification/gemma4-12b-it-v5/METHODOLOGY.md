...
For each predictive draw index, the generator selects exactly one component and exactly one target token length. It freezes both values before it generates a prompt. If the prompt or token identity is in the fixed historical exclusion inventory, it increments a case-local attempt nonce and regenerates only the prompt realization. It does not redraw the component. It does not redraw the target length. The accepted case keeps its original draw index. The generator does not use quota balancing. This conditional rejection law preserves the exact `1/24` component weights.
...
The earlier pre-freeze CMS sealing statement said that a byte-exact round-trip was verified. A CMS round-trip requires decryption. Issue #109 prohibits that operation. Therefore, that attempt is tainted and permanently ineligible (`manifests/tainted-pre-freeze-holdout-attempt.json`). A later replacement was made before the corrected historical-rejection law. It is superseded and ineligible for the final accepted freeze (`manifests/superseded-pre-correction-holdout-attempt.json`). The final holdout uses the corrected generator, the verified pinned tokenizer input, a fresh secret seed, and a fresh RSA-3072 keypair. It is CMS-sealed once. The correction uses only ciphertext structure, certificate, public-key DER, and hash checks. It does not decrypt the final holdout.
