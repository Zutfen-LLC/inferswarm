...
The earlier pre-freeze CMS sealing statement said that a byte-exact round-trip was verified. A CMS round-trip requires decryption. Issue #109 prohibits that operation. Therefore, that attempt is tainted and permanently ineligible (`manifests/tainted-pre-freeze-holdout-attempt.json`). Its seed, keypair, certificate, ciphertext, and plaintext identity were not reused. The replacement used the verified pinned tokenizer input, a fresh secret seed, and a fresh RSA-3072 keypair. It was CMS-sealed once. The correction used only ciphertext structure, certificate, public-key DER, and hash checks. It did not decrypt the replacement holdout.
