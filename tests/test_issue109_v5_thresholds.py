...
import issue109_v5_thresholds as thresholds
from issue109_v5_thresholds import derive_v5_threshold_artifacts
...
class Issue109V5ThresholdTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        # The real committed custody record is honestly incomplete (one
        # custodian). Exercising the full derivation happy path requires a
        # synthetic, otherwise-identically-shaped complete custody record;
        # the pinned-hash gate is patched to accept it for these tests only.
    ...
    def evidence(self):
        ...
        margins = {
            "schema": "inferswarm.issue109.v5-reference-margin-summary/1",
        ...
        selected = {
            "schema": "inferswarm.issue109.v5-selected-stress-eighth/1", "contract_id": CONTRACT_ID,
            "margin_definition": MARGIN_DEFINITION,
            ...
            "minimum_eligible_cases": 8, "eligible_case_count": V5_STRESS_POOL_CASES, "ineligible_case_count": 0,
            "ineligible_cases": [], "selected_count": 8, "selected": selected_rows,
            "state": V5_SELECTION_STATE,
        }
        self.assertEqual(selected["state"], V5_SELECTION_STATE)
        ...
        domain = {
            "schema": "inferswarm.issue109.v5-decision-domain-manifest/1",
            "contract_id": CONTRACT_ID,
            ...
            "statistical_cases": domain_rows[:V5_CALIBRATION_CASES], "stress_cases": domain_rows[V5_CALIBRATION_CASES:],
        }
        summary = {
            "schema": "inferswarm.issue109.v5-calibration-summary/1",
            "contract_id": CONTRACT_ID, "tooling_version": "inferswarm.issue109.v5-threshold-tooling/1",
            ...
            "statistical_cases": summary_rows[:V5_CALIBRATION_CASES], "stress_cases": summary_rows[V5_CALIBRATION_CASES:],
        }
        return corpus, pool, commitment, margins, selected, domain, summary

    def derive(self, *, custody_complete=True, **overrides):
        ...
        values = {
            ...
            "holdout_custody_record": copy.deepcopy(self.complete_custody if custody_complete else self.custody),
        }
        values.update(overrides)
        if custody_complete and "holdout_custody_record" not in overrides:
            with mock.patch.object(
                    thresholds, "V5_HOLDOUT_CUSTODY_RECORD_SHA256",
                    sha256_bytes(canonical_json_bytes(self.complete_custody))):
                return derive_v5_threshold_artifacts(**values)
        return derive_v5_threshold_artifacts(**values)

    def test_derives_three_core_limits_and_thirteen_telemetry_bands_from_complete_evidence(self):
        result = self.derive()
        core = result["core_threshold_manifest"]
        telemetry = result["telemetry_reference_bands"]
        self.assertEqual(len(core["limits"]), 3)
        self.assertEqual(len(telemetry["bands"]), 13)
        self.assertEqual(core["limits"]["decision_local_E_D"]["limit_hex"], float(1.0).hex())
        expected_max = float(V5_CALIBRATION_CASES + V5_SELECTED_STRESS_CASES).hex()
        self.assertEqual(core["limits"]["fp32-consumer-logits:max-absolute-difference"]["limit_hex"], expected_max)
    ...
    def test_rejects_the_currently_incomplete_real_custody_record(self):
        """The committed v5 custody record is honestly SEALED_CUSTODY_INCOMPLETE
        (one custodian). Threshold derivation must refuse against it as-is."""
        with self.assertRaisesRegex(MethodologyError, "custody is not complete"):
            self.derive(custody_complete=False)

    def test_deriver_binds_its_own_program_bytes(self):
        result = self.derive()
        self.assertEqual(result["core_threshold_manifest"]["provenance"]["derivation_program_sha256"], sha256_file(SCRIPTS / "issue109_v5_thresholds.py"))
    ...
    def test_mixture_membership_check_accepts_real_corpus_and_rejects_foreign_class(self):
        thresholds._mixture_membership(self.corpus["cases"], label="calibration corpus")
        bad = copy.deepcopy(self.corpus["cases"][:1])
        bad[0]["content_class"] = "not-a-real-class"
        with self.assertRaisesRegex(MethodologyError, "not a member of the frozen mixture population"):
            thresholds._mixture_membership(bad, label="calibration corpus")
    ...
    def test_explicit_deriver_negative_control_matrix(self):
        ...
        for label in ("core-demotion", "telemetry-promotion"):
            with self.subTest(label=label):
    ...
    def test_rejects_substituted_frozen_corpus_contract_or_holdout_identity(self):
        ...
        with self.assertRaisesRegex(MethodologyError, "holdout commitment hash mismatch"):
