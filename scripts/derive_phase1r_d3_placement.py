...
def main():
    ...
    local_remainder = [i for i in range(TOTAL) if i not in set(union)]
    ...
    artifact = {
        ...
        "geometry": {"num_moe_layers": LAYERS, "num_experts_per_layer": EXPERTS, "logical_expert_identities": TOTAL, "bytes_per_native_nvfp4_identity": BYTES, "gpu0_cache_slots": GPU0_SLOTS, "gpu0_cache_capacity_description": "runtime capacity; not logical ownership", "worker_a_slots": WORKER_SLOTS, "worker_b_slots": WORKER_SLOTS, "worker_resident_bytes_each": WORKER_SLOTS * BYTES, "combined_worker_resident_bytes": 2 * WORKER_SLOTS * BYTES},
        "semantics": {"gpu0_local_identity_count": len(local_remainder), "gpu0_cache_capacity_slots": GPU0_SLOTS, "gpu0_cache_kind": "dynamic", "gpu0_cache_policy": "FreeToken runtime cache", "gpu0_cache_is_logical_ownership": False, "logical_owner_rule": "worker A if in worker_a; else worker B if in worker_b; else GPU0 local"},
        "partition": {"algorithm": "rank-order deterministic greedy; lexicographically minimize maximum W1-W4 normalized A/B load difference, sum of differences, total normalized mass difference, then worker ID A before B; capacity override for a sole non-full worker", "worker_a": {"flat_ids_in_rank_order": a, "identities": identities(a), "per_layer": by_layer(a)}, "worker_b": {"flat_ids_in_rank_order": b, "identities": identities(b), "per_layer": by_layer(b)}, "local_remainder": {"flat_ids": local_remainder, "identities": identities(local_remainder), "per_layer": by_layer(local_remainder)}},
        "validation": {"union_slots": len(union), "unique_union_slots": len(set(union)), "worker_sets_disjoint": not bool(set(a) & set(b)), "local_remainder_slots": len(local_remainder), "logical_owner_sets_pairwise_disjoint": not bool(set(a) & set(b)) and not bool(set(a) & set(local_remainder)) and not bool(set(b) & set(local_remainder)), "logical_owner_partition_covers_all_identities": set(a) | set(b) | set(local_remainder) == set(range(TOTAL)), "flat_id_arithmetic": "flat_id = layer * 256 + expert_id", "worker_a_route_mass": a_mass, "worker_b_route_mass": b_mass, "remote_union_route_mass": remote, "local_remainder_route_mass": local, "ab_balance": {"absolute_normalized_difference_per_class": differences, "maximum": max(differences.values()), "sum": sum(differences.values()), "total_normalized_mass_difference": abs(sum(a_mass["per_class"][c]["normalized_mass"] for c in CLASSES) - sum(b_mass["per_class"][c]["normalized_mass"] for c in CLASSES))}}
    }
