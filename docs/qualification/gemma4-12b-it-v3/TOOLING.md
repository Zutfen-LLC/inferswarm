...
| `scripts/verify_issue86_v3_unseal.py` | v3 unseal preflight — never decrypts, stops before decrypt. The CLI REQUIRES `--holdout-ciphertext` and `--recipient-certificate` file paths and hashes the ACTUAL bytes of the supplied files; missing files fail unreadable, corrupt/wrong/historical material fails the identity checks (no frozen-hash substitution anywhere in the CLI path). The threshold manifest is likewise hashed from its ACTUAL file bytes (`sha256_file(args.threshold_manifest)`, the accepted #79 behavior) while the verifier separately enforces canonical-representation equality — so both the exact committed bytes and the canonical form are bound, and a whitespace-only reformat of the threshold file fails the preflight |
