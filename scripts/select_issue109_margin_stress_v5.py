#!/usr/bin/env python3
"""Select the eight v5 stress cases from reference-only margins (issue #109).

Eligibility and selection rule are unchanged from the accepted v4 (issue
#86/#95) rule:

- non-finite margin (NaN/+Inf/-Inf): unconditional reference failure, fails
  closed immediately (NONFINITE_REFERENCE_MARGIN);
- finite NEGATIVE margin: unconditional reference/order-consistency failure,
  fails closed immediately (NEGATIVE_REFERENCE_MARGIN);
...
four largest finite margins; deterministic tie-break by case_id. Requires at
least 8 eligible cases. This module is pure selection logic: it does not
execute a model and only consumes already-computed reference margins,
generated in a later physical campaign (issue #110).
"""
...
def select(pool: dict[str, Any], margins: dict[str, Any], commitment: dict[str, Any]) -> dict[str, Any]:
    ...
    if margins.get("schema") != V5_MARGIN_SUMMARY_SCHEMA:
        raise ValueError("v5 reference margin summary schema mismatch")
    ...
    if commitment.get("schema") != V5_STRESS_COMMITMENT_SCHEMA:
        raise ValueError("v5 stress selection commitment schema mismatch")
    if commitment.get("contract_id") != CONTRACT_ID:
        raise ValueError("v5 stress selection commitment contract mismatch")
    if commitment.get("state") != V5_COMMITMENT_STATE:
        raise ValueError("v5 stress selection rule was not committed before reference execution")
    if commitment.get("candidate_pool_sha256") != pool_hash:
        raise ValueError("v5 stress selection commitment does not bind the exact frozen pool")
    if commitment.get("candidate_observations_forbidden") is not True:
        raise ValueError("v5 stress selection commitment does not forbid candidate observations")
    if commitment.get("selected_case_count") != 8:
        raise ValueError("v5 stress selection commitment must require eight cases")
    if commitment.get("selection_program") != "scripts/select_issue109_margin_stress_v5.py":
        raise ValueError("v5 stress selection commitment program path mismatch")
    if commitment.get("selection_program_sha256") != sha256_file(
        Path(__file__).resolve().parent / "select_issue109_margin_stress_v5.py"
    ):
        raise ValueError("v5 stress selection commitment program hash mismatch")
    if commitment.get("eligibility_rule") != V5_ELIGIBILITY:
        raise ValueError("v5 stress selection commitment eligibility rule mismatch")
    if commitment.get("minimum_eligible_cases") != MIN_ELIGIBLE:
        raise ValueError("v5 stress selection commitment minimum eligible count mismatch")
    if commitment.get("margin_definition") != MARGIN_DEFINITION:
        raise ValueError("v5 commitment margin definition is not the unchanged min-over-8 definition")
    if commitment.get("selection_rule") != V5_SELECTION_RULE:
        raise ValueError("v5 commitment selection rule mismatch")
    if margins.get("stress_pool_sha256") != pool_hash:
        raise ValueError("v5 reference margins do not bind the exact frozen pool")
    pool_cases = {row["case_id"]: row for row in pool["cases"]}
    rows = margins.get("cases")
    if not isinstance(rows, list) or not rows:
        raise ValueError("v5 reference margins must be a nonempty list")
    if {row.get("case_id") for row in rows} != set(pool_cases) or len(rows) != len(pool_cases):
        raise ValueError("v5 reference margins must cover each pool case exactly once")
    ...
    for row in rows:
        ...
        if case_id in seen_ids:
            raise ValueError(f"v5 reference margins contain duplicate case ID {case_id!r}")
    ...
    for row in rows:
        ...
        if not math.isfinite(margin):
            raise NonfiniteReferenceMarginError(
                ...
                f"produced margin {margin!r}; selection stops unconditionally"
            )
        if margin < 0.0:
            raise NegativeReferenceMarginError(
                f"NEGATIVE_REFERENCE_MARGIN: reference case {row['case_id']!r} produced "
                f"finite negative margin {margin!r}; selection stops unconditionally"
            )
        eligible.append((margin, row["case_id"]))
    if len(eligible) < MIN_ELIGIBLE:
        raise ValueError(
            f"v5 stress selection requires at least {MIN_ELIGIBLE} eligible cases; "
    ...
    return {
        "schema": V5_SELECTED_EIGHT_SCHEMA,
